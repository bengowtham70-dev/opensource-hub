...
import { Sparkles, KeyRound, LoaderCircle, Send, Bot, Search } from "lucide-react";
    ...
    <div className="mx-auto max-w-[900px] px-4 md:px-6 py-10">
      <header className="mb-6">
        <h1 className="font-display text-display-lg flex items-center gap-3">
          <Sparkles size={28} className="text-primary" /> AI Tool Finder
        </h1>
      ...
      <form onSubmit={run} className="card-glass p-5 flex flex-col gap-3">
        <textarea
          value={task}
          ...
          className="w-full card-glass !bg-elevated p-3.5 text-[14.5px] text-ink placeholder:text-faint outline-none focus:border-primary/50 resize-y"
        />
          ...
          <div className="mt-1 p-4 rounded-xl border border-line bg-elevated/50 grid sm:grid-cols-[1fr_180px_140px] gap-2.5 animate-card-in opacity-0">
            <label className="text-[12px] text-faint flex flex-col gap-1">
              ...
              <input
                ...
                placeholder="sk-…"
                className="card-glass !bg-elevated px-3 py-2 text-[13px] text-ink tnum outline-none focus:border-primary/50"
              />
            </label>
            <label className="text-[12px] text-faint flex flex-col gap-1">
              Base URL
            ...
            {result.results.map((r, i) =>
              r.pairing ? (
                <Link
                  key={r.repo}
                  to={`/repo/${r.repo}`}
                  className="card-glass p-5 flex items-start gap-4 group animate-card-in"
                  style={{ animationDelay: `${i * 40}ms`, opacity: 0 }}
                >
                  <span className="tnum text-[12px] text-primary mt-1">{String(i + 1).padStart(2, "0")}</span>
        ...
        <div className="mt-6 grid sm:grid-cols-3 gap-3">
          {[
            ...
            <button
              ...
              type="button"
              onClick={() => setTask(ex)}
              className="btn-tactile card-glass p-4 text-left text-[13px] text-dim hover:text-ink flex items-start gap-2"
            >
              <Search size={14} className="text-primary shrink-0 mt-0.5" /> {ex}
            </button>
          ))}
        </div>
      )}
